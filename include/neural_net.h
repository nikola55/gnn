...
namespace gnn {
	...
	*/
	...
	double cost(const gnn::nnDef &def,
	...
	*
	* @param1 definition
	* @param2 computed activations for each layer including the
    *         training set ( n(layers) ) { &ts[0], &A1[0], ... &AN[0] }
	* @param3 activations with bias term added including the
    *         training set ( n(layers) ) { &ts[0], &A1[0], ... &AN[0] }
